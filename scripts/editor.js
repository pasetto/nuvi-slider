function nuvi_slider_init_editor() {
	var sliderConfig = document.querySelectorAll(".nuvi_slider_editor");
	for (var slideNr = 0; slideNr < sliderConfig.length; ++slideNr) {
		...
		var dotIcon = slideObj.getAttribute("data-dot-icon") || 0;
		var dotIconActive = slideObj.getAttribute("data-dot-icon-active") || 0;
		var dotClickable = slideObj.getAttribute("data-dot-clickable") || 0;
		...
		if (pagination) {
			newContent += '<div class="nuvi-pagination" style="bottom:' + dotPadding + 'px"></div>';
			paginationOptions = {
				el: '.nuvi-pagination',
				bulletClass: 'nuvi-pagination-bullet',
				bulletActiveClass: 'nuvi-pagination-active',
				clickable: dotClickable == 1,
				// type: 'bullets',
				// el: '.swiper-pagination',
				type: 'bullets',
				renderBullet: function (index, className) {
					return (dotIcon == 0) ? '<span class="' + className + '"></span>' : '<span class="' + className + '"><i class="' + dotIcon + ' default"></i> <i class="' + dotIconActive + ' active"></i></span>';
...
if (document.readyState !== 'loading') {
	...
	document.addEventListener('DOMContentLoaded', nuviSliderDomReady);
}

function nuviSliderDomReady() {
	if (document.querySelectorAll(".nuvi_slider_editor") && document.querySelectorAll(".nuvi_slider_editor").length > 0) {
		nuvi_slider_init_editor();
	}
}
console.log('Nuvi SLider Ready');
